...
float *d_buf1, float *d_buf2, const int rank, const int world_size, const long segments_per_process)
{
...
#pragma omp parallel for simd
    ...
    if(rank == world_size - 1){
      MPI_Recv(&d_t[start_segment - 1], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      ...
      MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }else{
      MPI_Recv(&d_t[start_segment - 1], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Send(&d_t[last_segment - 1], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
      MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
